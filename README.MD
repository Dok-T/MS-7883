...
You can see if your motherboard is compatible by crossflashing it using external EEPROM programmer, or flashrom under linux.

- X99A GAMING 9 ACK (https://github.com/Dok-T/MS-7883/issues/7)
    Reported as working, but without any further informations (such as WiFi, PXE, SATA, PCIE, etc)

**Update: 07/26/2025**

## I can sadly confirm that board doesn't support ECC "mode" at all, but can accept ECC Reg as I previously listed here.

**Update: 07/20/2025**
## 🧪 BIOS ECC Test Report – v0.4.1_RC

## ECC Support Status


The ECC memory menu is currently **unlocked in BIOS**, but support is still **experimental and under testing**.  
At the moment, ECC appears to be **non-functional**, as indicated by the following `dmesg` log excerpt:
...
### ✅ Working RAM Configurations:
- ✅ **2Rx8** and **1Rx8** **RDIMM** modules are recognized and work reliably. (at least 16GB per RAM stick. Not tested higher than 128GB of total RAM)
- ❌ **2Rx4** and **1Rx4** **RDIMM** modules are **not supported and not working**. (don't waste trying, not working at all)
- ❌ **LRDIMM** modules do **not work** at all.
- ❔ **Unbuffered UDIMM ECC** Not tested, feedback are welcome!

> **Tested with:**  
Intel Xeon E5-2683 v4 + Samsung 2Rx8 16GB x8  
Module ref: `M393A2K43CB2-CVF` (total 128 GB)
...
- MSI X99A GODLIKE Carbon ~~(to be confirmed, should be 100% compatible)~~ Working (thanks to codeyoc / https://www.reddit.com/r/MSI_Gaming/comments/1cicehe/comment/mys10e4/?context=3 )

```MSI X99 Carbon Godlike - YES you CAN

Just successfully did this myself after M-Flash kept giving me the "Warning: the BIOS you selected is not for this MB" error AND wouldn't even detect my BIOS file. This typically happens with modded BIOS files that M-Flash doesn't recognize.

Solution: FreeDOS + AFUDOS with bypass flag
...
What you need:
    ...
    AFUDOS utility (AMI Firmware Update tool)

    Your BIOS file

Steps:

    Create FreeDOS bootable USB with Rufus

    Download AFUDOS (search "AMI Firmware Update Utility")

    Copy AFUDOS.exe and your BIOS file to USB root

    Rename BIOS file to something simple like "BIOS.ROM" (DOS doesn't like complex filenames)

    Boot from USB to FreeDOS

    Run command: AFUDOS BIOS.ROM /X

The magic is the /X flag - it means "Don't Check ROM ID" which bypasses the compatibility check that M-Flash uses.

⚠️ Standard warnings apply - know what you're doing, stable power, etc. But if you're confident the BIOS is actually compatible (like with modded BIOS files), this works perfectly.

Just flashed my DokT BIOS mod successfully using this method. M-Flash refused it and couldn't even see the file, but AFUDOS with /X said "I don't care about your warnings" and flashed it perfectly.
```
...
### Know bugs / not tested / patched :

- ~~Intel AMT not tested (Not tested. Seems legit as this is not an server chipset)~~ Intel MEBx / AMT Not working at all, even after using the 5M ME FW.
- ~~Xeon bug in UEFI/BIOS~~ **Patched in 0.3.7 thanks to Sweet_Kitten (This mean you'll can flash newer BIOSes using M-FLASH without swapping CPU for a core i7. Using Xeon CPU are now supported)**
- you tell me  (lol)
...
### Version 0.4.1 (v0.4.1_RC)
### SATA
- ✅ **Enabled Hot Plug for sSATA + SATA**  
  Allows drives to be connected/disconnected while the system is running.
- ✅ **Enabled AHCI LPM (Link Power Management) for SATA + sSATA**  
  Enables power-saving modes for SATA drives (partial/slumber states).

### CPU
- ✅ **Enabled xTPR**  
  Enables xTPR Message Disable, useful for virtualization.
- ✅ **Enabled SMX (Safer Mode Extensions)**  
  Enable Safer Mode Extensions CPU Instruction
- ✅ **Enabled X2APIC**  
  Extended APIC mode for better multiprocessor performance.
- ✅ **Enabled SMM Save State**  
  Improves power management and system compatibility.
- ✅ **Enabled Targeted SMI**  
  Optimized system management interrupts in multi-socket setups.

### RAM
- ✅ **Fully Unlocked Memory Options**  
  Unlocks hidden RAM settings: timings, training, RTL/IOL tuning, etc.

### Other
- ✅ **SLIC Enabled**  
  Adds OEM SLIC 2.1/2.3 table for automatic Windows activation (e.g., Windows 7 Pro).
- ✅ **Enabled Wake-on-LAN (WoL)**  
  Allows the system to be powered on remotely via network.
- ✅ **Updated BIOS Strings & Help Descriptions**  
  Improved text and descriptions in BIOS UI.
- ✅ **Updated BIOS Revision & Build Date**  
  Clear versioning for the modified BIOS.

### Version 0.4.2

*Based on 0.4.1*
- tried to use 5M SKU version of Intel ME FW. (9.1.45.3000) for testing and trying to make REG ECC working, but no success. Plus, using this Intel ME FW make impossible to boot from USB (instant reboot, but POSTING and booting to UEFI was OK)

- Not releasing it for obvious reasons

### Version 0.4.3

*based on 0.4.1*
- Updated Intel ME FW from 9.1.37.1002 to 9.1.40.1000 (no more USB boot bug AFAIK) (still 1.5M SKU FW version)
- Fixed some Intel ME String in RC Setup
- Enabled Ram Power Management
- Disabled "Lock Chipset" in IntelRCSetup -> Processor Configuration
- Disabled SecureBoot by default (you still can enable it later, usefull when rebooting a linux server and avoiding multiple reboots into UEFI for disabling it)
- Enabled PSMI
- small fixes
